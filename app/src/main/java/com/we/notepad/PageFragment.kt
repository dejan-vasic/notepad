...
//        val adapter = PageAdapter(letDirectory)
        ...
        val letDirectory = File(path, "LET")
        letDirectory.mkdirs()
        val dir = File(letDirectory, param1!!)
        val adapter = PageAdapter(dir)
        viewPager2.adapter = adapter
        button.setOnClickListener {
            val file = File(dir, "page${dir.listFiles()!!.size}.txt")
            file.appendText("")
            adapter.notifyItemInserted(dir.listFiles()!!.size - 1)
        }
    }
